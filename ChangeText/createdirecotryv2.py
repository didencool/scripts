...
# Словник для визначення, які файли копіювати і як перейменувати
# Формат: {'вихідна_назва.tap': 'цільова_назва.nc'}
files_to_copy = {
    "обрезка.tap": "cut.nc",
...
def copy_and_rename_files(file_map, destination_dir):
    ...
    for source_name, target_name in file_map.items():
        ...
        try:
            # Використовуємо shutil.copy2 для копіювання з збереженням метаданих, 
            # і копіюємо безпосередньо в цільовий шлях з новою назвою.
...
copy_and_rename_files(files_to_copy, DRAFT_DIR)
